...
- Locally observed native MCP execution jobs appear as standard generic tool cards, named `opencode_mcp.<native tool name>`. Cards include sanitized arguments/actions, the exact native status, and safe final output or error text. `__display_status` is an explicitly display-only input field, not an argument sent to the native tool. It makes running/completed/error-related states visible even though the pinned generic renderer does not show its state title.
