...
test("plugin startup → SDK → simulated Notion → real HTTP MCP → native edits, without approvals", async t => {
  ...
  t.after(async()=>{ await mcp?.close(); await runtime?.close(); await runtime?.shared.stopIfUnused(); await rm(temp,{recursive:true,force:true}) })
  ...
  const records=[], changes=[], calls=[]; let executionScope
  ...
  async function tool(name,args){
    ...
    let job=unpack(await mcp.callTool({name,arguments:{...executionScope,arguments:args}}))
    for(let i=0;i<30&&job.status==="running";i++) job=unpack(await mcp.callTool({name:"opencode_job_result",arguments:{...executionScope,job_id:job.job_id,wait_seconds:1}}))
  ...
  const factory=()=>({withTimeout:async(_ms,action)=>action(),client:{account:async()=>account,mcp:()=>manager},interrupt:async()=>{},send:async input=>{
    calls.push(input); executionScope=input.executionScope
  ...
  await mcp.close();mcp=undefined;await runtime.close();assert.equal((await fetch(`http://127.0.0.1:${port}/healthz`)).status,200)
